...
apps/api/src/modules/
  ...
  reports/         No model of its own — 28 reports read live from every other module via aggregation pipelines — **implemented** (Phase 10); calls into orders/, b2b/, inventory/, manufacturing/, accounting/ (read-only), never the reverse — see architecture.md §5F
...
## 5F. Reporting — **implemented** (Phase 10)

`apps/api/src/modules/reports`, no model of its own — every report is computed live from other modules' collections, through a MongoDB aggregation pipeline. 28 reports across Sales/Inventory/Gold/B2B/Profitability, all served by one registry-driven framework rather than 28 bespoke endpoints.

- **A registry, not 28 hand-built screens.** `report-registry.ts` is an array of `ReportDefinition`s (title, description, which of the shared filters apply, columns with their display format, whether it's a point-in-time snapshot). `reports.service.ts`'s `RUNNERS` dispatch table maps each key to one function; `runReport` resolves the shared filter context once (`resolveReportContext`, reusing the dashboard's own `resolveRange`/`resolveScope` — CLAUDE.md rule 1's "one engine, different inputs" discipline, extended past pricing to date/branch/location resolution generally) and stamps the registry's own display metadata onto whatever the report function returns. The ERP has exactly one report screen (`ReportView`), driven entirely by the registry — adding a report never means writing frontend code.
- **Real aggregations, never a collection dump.** A grouped report (`$group` by day/status/category) or a paginated detail list (`$facet` with `$skip`/`$limit` inside the pipeline) — either way, what reaches Node is small. Gold's movement reports join the ledger to `InventoryItem` with a `$lookup` inside the pipeline rather than fetching item ids into an array first.
- **Scoped honestly, never invented.** `Order`/`Invoice` carry no location field, so a branch/location-scoped sales report derives it from the real `SALE` ledger entry's `sourceLocationId` via `Transaction` — the same "don't invent a metric the system doesn't have" discipline as the dashboard.
- **Weight read from the right place.** Material/job-work issue always moves a whole `InventoryItem` (§15.3), so for most movement types the ledger entry's own `grossWeight`/`fineWeight` are a *delta* of 0 (the piece's weight didn't change, only its custody/status) — the gold reports read `balanceAfter.grossWeight`/`fineWeight` instead, the piece's real weight at that event. Wastage isn't a ledger movement (§15.5); that report reads Manufacturing's own `reconciliation` directly.
- **Reuse over re-derivation.** B2B's outstanding/ageing reports are thin wrappers over Accounting's `receivables-reads.service.ts` (§5E); profitability's cost of goods sold comes from `InventoryItem.cost` via the `SALE` ledger — the same real figure the accounting posting engine would use, computed independently since GL sales posting doesn't yet cover B2C.
- **Export without the memory cost.** `/api/reports/:key/export` calls the exact same paginated function with `pageSize` set to a fixed cap (10,000) and streams a CSV (`Content-Disposition: attachment`) — never a raw JSON array the browser has to hold and render in full.
