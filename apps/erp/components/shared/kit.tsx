...
import { Alert, Skeleton, cn } from "@jewellery/ui";
import { errorMessage } from "../../lib/api/queries";

/**
 * The small table/form/loading primitives every workflow module's own `shared.tsx` needs
 * (accounting, b2b, exchange, hallmarking, manufacturing, purchasing, repair, returns) — one
 * definition instead of eight byte-identical copies, so a spacing or style change is one edit,
 * not eight. Each module's own `shared.tsx` re-exports these and keeps its module-specific
 * pieces (status tone map, currency/weight formatting) local, since those genuinely differ.
 */
