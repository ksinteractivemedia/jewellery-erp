...
import { allocatePaymentSchema, approvePurchaseOrderSchema, creditApprovalSchema, quoteSchema, recordPaymentSchema, rejectSchema, reversePaymentSchema, updateB2BProfileSchema, verifyB2BPaymentSchema } from "@jewellery/validation";
...
export function createB2BAdminRouter(deps: { authenticate: RequestHandler; b2b: B2BModule }) {
  const { reads, procurement, fulfilment, payments } = deps.b2b;
  const router = Router();
  ...
  router.post("/purchase-orders/:id/approve", decide, validateBody(approvePurchaseOrderSchema), asyncHandler(async (req, res) => {
    res.status(201).json({ order: await procurement.approvePurchaseOrder(param(req), actor(req), { ...(req.body.creditOverride ? { creditOverride: req.body.creditOverride } : {}), canOverrideCredit: canOverride(req) }) });
  }));
  ...
  router.post("/orders/:id/invoice", decide, asyncHandler(async (req, res) => void res.status(201).json({ invoice: await fulfilment.invoice(param(req), actor(req)) })));
