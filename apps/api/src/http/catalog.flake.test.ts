...
const api = (method: "get" | "post" | "put" | "patch" | "delete", url: string, token = manager) => {
  const timer = setTimeout(() => console.log("SLOW>5s", method, url), 5000);
  const req = request(t.app)[method](url).set(bearer(token));
  const origThen = req.then.bind(req);
  (req as any).then = (a: any, b: any) => origThen((r: any) => { clearTimeout(timer); return a?.(r); }, (e: any) => { clearTimeout(timer); return b?.(e); });
  return req;
};
