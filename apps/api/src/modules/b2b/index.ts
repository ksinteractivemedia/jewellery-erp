...
export function createB2BModule(deps: { media: MediaService; now?: () => Date }) {
  const base = { ...(deps.now ? { now: deps.now } : {}) };
  return {
    procurement: createProcurementService({ media: deps.media, ...base }),
    fulfilment: createFulfilmentService(base),
    payments: createB2BPaymentService(base),
    reads: createB2BReads({ media: deps.media, ...base }),
  };
}
